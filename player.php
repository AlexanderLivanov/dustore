...
<head>
    ...
    <style>
        ...
        .avatar-frame   { display:block; }
        .edit-profile-btn {
            position:absolute; bottom:-22vh; left:19.1vh; transform:translateX(-50%);
            background:rgba(30,20,40,.92); border:1px solid rgba(255,255,255,.18);
            color:#fff; border-radius:20px; padding:5px 14px; font-size:12px;
        ...
        <div class="profile-right" style="flex:1;min-width:280px;color:white;">
                ...
                <div>
                    ...
                    if (!empty($_SESSION['USERDATA']['id'])) {
                        ...
                        if ($is_online_launcher || $is_online_web) {
                            ...
                            $status_text = '<small style="color:gray;font-size:15px;">● Был '.format_last_seen($last_active).'</small>';
                        }
                    }
                    ?>
                    <h1><?= htmlspecialchars($user['first_name'].' '.$user['last_name']) ?> <?= $status_text ?></h1>
                    <p><light>На платформе с:</light> <?= date('d.m.Y', strtotime($user['added'])) ?></p>
                </div>
                ...
                <div class="profile-info-card">
                    <div class="pf-wrap">
                        <div class="pf-stats-row">
                            <div class="pf-stat"><span class="pf-stat-num"><?= (int)$stats['library_count'] ?></span><span class="pf-stat-lbl">игр</span></div>
                            <div class="pf-stat-div"></div>
                            <div class="pf-stat"><span class="pf-stat-num"><?= (int)$stats['friends_count'] ?></span><span class="pf-stat-lbl">друзей</span></div>
                            <div class="pf-stat-div"></div>
                            <div class="pf-stat"><span class="pf-stat-num"><?= (int)$stats['reviews_count'] ?></span><span class="pf-stat-lbl">отзывов</span></div>
                            <div class="pf-stat-div"></div>
                            <div class="pf-stat"><span class="pf-stat-num"><?= (int)$stats['achievements_count'] ?></span><span class="pf-stat-lbl">достижений</span></div>
                            <div class="pf-meta">
                        ...
                        <a href="/l4t/<?= htmlspecialchars($username) ?>" class="pf-l4t-btn">
                            <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><path d="M12 6h-6a2 2 0 0 0-2 2v10a2 2 0 0 0 2 2h10a2 2 0 0 0 2-2v-6"/><path d="M11 13l9-9"/><path d="M15 4h5v5"/></svg>
                            Профиль на L4T
                            <span class="pf-l4t-badge">internal</span>
                        </a>
                        ...
                        <div class="pf-section">
                            <div class="pf-section-head"><span class="pf-section-title">Достижения</span></div>
                            <div class="achievements-container" style="display:flex;flex-wrap:wrap;gap:8px;margin-top:8px;">
                                ...
                                <div class="achievement-icon" title="<?= $title ?>"
                                     onclick='showAchievementModal({title:<?= json_encode($title) ?>,description:<?= json_encode($ach["description"]??"") ?>,icon:<?= json_encode($ach["icon_url"]??"🏆") ?>,date:<?= json_encode($ach["awarded_at"]??"") ?>})'>
                                    <?php if (!empty($ach["icon_url"])): ?><img src="<?= htmlspecialchars($ach["icon_url"]) ?>" alt="<?= $title ?>" class="achievement-icon-img"><?php else: ?>🏆<?php endif; ?>
                                </div>
                                <?php endforeach; ?>
                            </div>
                        </div>
                        ...
                        <div class="pf-section">
                            <div class="pf-section-head">
                                ...
                                <?php if (count($games) > 6): ?><button onclick="switchTab('games')" class="pf-link-btn">Все игры →</button><?php endif; ?>
                            </div>
                            <div class="pf-games-scroll">
                                ...
                                <a href="/g/<?= $game['id'] ?>" class="pf-game-thumb">
                                    <div class="pf-game-cover">
                                        ...
                                        <div class="pf-game-overlay"><span class="pf-game-rating">★ <?= number_format($game['rating'],1) ?></span></div>
                                    </div>
                        ...
                        <div class="pf-section">
                            <div class="pf-section-head">
                                ...
                                <?php if (count($reviews) > 3): ?><button onclick="switchTab('reviews')" class="pf-link-btn">Все отзывы →</button><?php endif; ?>
                            </div>
                ...
                <div class="profile-card games-profile-card">
                    <div class="games-section-header">
                        ...
                        <div class="games-view-icons">
                            <button class="games-view-icon-btn" data-view="showcase" title="Витрина">
                                <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><rect x="3" y="3" width="7" height="7"/><rect x="14" y="3" width="7" height="7"/><rect x="3" y="14" width="7" height="7"/><rect x="14" y="14" width="7" height="7"/></svg>
                            </button>
                            <button class="games-view-icon-btn active" data-view="carousel" title="Карусель">
                                <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><rect x="3" y="3" width="7" height="18"/><rect x="14" y="3" width="7" height="18"/></svg>
                            </button>
                        </div>
                    </div>

                    <div class="carousel-3d-container" id="carouselView" style="display:block;">
                        <button class="carousel-3d-btn prev">‹</button>
                                ...
                                <div class="carousel-3d-item" data-index="<?= $index ?>">
                                    <a href="/g/<?= $game['id'] ?>" class="game-card-link" style="text-decoration:none;color:inherit;">
                                        <div class="game-card-vertical">
                                            <img src="<?= !empty($game['path_to_cover']) ? htmlspecialchars($game['path_to_cover']) : '/assets/default-game-cover.png' ?>" alt="Обложка" class="game-cover-vertical">
                        ...
                        <button class="carousel-3d-btn next">›</button>
                    </div>
                    ...
                    <div class="showcase-container" id="showcaseView" style="display:none;">
                        <div class="showcase-body" id="showcaseGamesBody">
                            <div class="showcase-shelves" id="showcaseShelves">
                                ...
                                $perRow = 8;
                                $slots  = $games;
                                $rem    = count($slots) % $perRow;
                                if ($rem) $slots = array_merge($slots, array_fill(0, $perRow-$rem, null));
                                if (empty($slots)) $slots = array_fill(0, $perRow, null);
                                foreach (array_chunk($slots, $perRow) as $row):
                                ?>
                                <div class="showcase-shelf-row">
                                    <?php foreach ($row as $sg): ?>
                                    <?php if ($sg): ?>
                                    <div class="showcase-game-card" onclick="window.location.href='/g/<?= $sg['id'] ?>'" title="<?= htmlspecialchars($sg['name']) ?>">
                                        <?php if (!empty($sg['path_to_cover'])): ?><img src="<?= htmlspecialchars($sg['path_to_cover']) ?>" alt=""><?php endif; ?>
                                    </div>
                                    <?php else: ?>
                        ...
                        <div class="showcase-achievements-body" id="showcaseAchievementsBody" style="display:none;">
                            <?php if (empty($achievements)): ?>
                            <div class="showcase-wip-inner"><p class="showcase-wip-title">Достижений пока нет</p><p class="showcase-wip-sub">Играй, исследуй и зарабатывай награды</p></div>
                            <?php else: ?>
                            <div class="achievements-grid">
                                <?php foreach ($achievements as $ach):
                                    $title = htmlspecialchars($ach['name']); ?>
                                <div class="achievement-card" onclick='showAchievementModal({title:<?= json_encode($title) ?>,description:<?= json_encode($ach["description"]??"") ?>,icon:<?= json_encode($ach["icon_url"]??"🏆") ?>,date:<?= json_encode($ach["awarded_at"]??"") ?>})'>
                                    <div class="achievement-card-icon"><?php if (!empty($ach["icon_url"])): ?><img src="<?= htmlspecialchars($ach["icon_url"]) ?>" alt=""><?php else: ?>🏆<?php endif; ?></div>
                                    <div class="achievement-card-name"><?= $title ?></div>
                                    <?php if (!empty($ach['awarded_at'])): ?><div class="achievement-card-date"><?= date('d.m.Y', strtotime($ach['awarded_at'])) ?></div><?php endif; ?>
                                </div>
                                <?php endforeach; ?>
                            </div>
                            <?php endif; ?>
            ...
            <div id="tab-reviews" class="tab-content">
                <div class="profile-card">
                        ...
                        <a href="/g/<?= $review['game_id'] ?>" class="review-item-link" style="text-decoration:none;color:inherit;">
                            <div class="review-item">
                                <div class="review-header">
                                    <img src="<?= !empty($review['game_cover']) ? htmlspecialchars($review['game_cover']) : '/assets/default-game-cover.png' ?>" alt="" class="review-game-cover">
                                    <div>
                    ...
                    <?php else: ?>
                    <div class="empty-state"><p>Пользователь пока не оставил ни одного отзыва</p></div>
                    <?php endif; ?>
                </div>
            </div>
                        ...
                        <a href="/player/<?= htmlspecialchars($friend['username']) ?>"
                           style="display:flex;align-items:center;gap:14px;background:rgba(255,255,255,.04);border-radius:12px;padding:12px 16px;text-decoration:none;color:white;transition:background .2s;">
                            ...
                            <div style="flex:1;min-width:0;">
                                <div style="font-weight:600;font-size:.97em;"><?= htmlspecialchars(trim($friend['first_name'].' '.$friend['last_name'])) ?></div>
                                <div style="font-size:.82em;color:#888;margin-top:2px;">@<?= htmlspecialchars($friend['username']) ?></div>
                            </div>
                            <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#555" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="9 18 15 12 9 6"/></svg>
                        </a>
                        <?php endforeach; ?>
                    </div>
                    <?php endif; ?>
            ...
            <div id="tab-developer" class="tab-content">
                <div class="profile-card">
                    ...
                    <?php else: ?>
                    <div class="empty-state"><p>Этот раздел доступен только владельцу профиля.</p></div>
                    <?php endif; ?>
                </div>
            </div>
...
<div class="modal" id="achievementModal" style="display:none;position:fixed;top:0;left:0;right:0;bottom:0;background:rgba(0,0,0,.9);backdrop-filter:blur(10px);z-index:1000;align-items:center;justify-content:center;">
    <div class="modal-content" style="background:linear-gradient(135deg,#2a3344,#1a1f2e);border-radius:20px;padding:30px;max-width:500px;width:90%;position:relative;box-shadow:0 25px 60px rgba(0,0,0,.9);border:2px solid rgba(255,215,0,.3);">
        <button onclick="closeAchievementModal()" style="position:absolute;top:15px;right:15px;background:rgba(255,215,0,.2);border:2px solid #ffd700;color:#ffd700;width:35px;height:35px;border-radius:50%;cursor:pointer;font-size:20px;">&times;</button>
        <div id="achievementModalBody"></div>
    </div>
</div>

<!-- Модалка предметов -->
<div class="modal" id="itemModal" style="display:none;position:fixed;top:0;left:0;right:0;bottom:0;background:rgba(0,0,0,.9);backdrop-filter:blur(10px);z-index:1000;align-items:center;justify-content:center;">
    <div class="modal-content" style="background:linear-gradient(135deg,#2a3344,#1a1f2e);border-radius:20px;padding:40px;max-width:600px;width:90%;position:relative;box-shadow:0 25px 60px rgba(0,0,0,.9);border:2px solid rgba(0,245,255,.3);">
        <button onclick="closeItemModal()" style="position:absolute;top:20px;right:20px;background:rgba(255,0,110,.2);border:2px solid #ff006e;color:#ff006e;width:40px;height:40px;border-radius:50%;cursor:pointer;font-size:24px;display:flex;align-items:center;justify-content:center;z-index:1001;">&times;</button>
...
function showAchievementModal(ach) {
    ...
    <div style="text-align:center;">
        <div style="font-size:3em;margin-bottom:15px;"><img src="${ach.icon}" alt="${ach.title}" class="achievement-icon-img"></div>
        ...
        <div style="font-size:.9em;color:#888;">Дата получения: ${ach.date||''}</div>
        <button style="margin-top:20px;padding:10px 20px;border:none;border-radius:10px;background:linear-gradient(135deg,#ffd700,#ffbe0b);color:#000;font-weight:bold;cursor:pointer;" onclick="closeAchievementModal()">Закрыть</button>
    </div>`;
    document.getElementById('achievementModal').style.display = 'flex';
}
function closeAchievementModal() { document.getElementById('achievementModal').style.display = 'none'; }
document.getElementById('achievementModal').addEventListener('click', e => { if(e.target===e.currentTarget) closeAchievementModal(); });
...
document.getElementById('friendActionBtn')?.addEventListener('click', async function() {
    ...
    try {
        ...
        const map = {
            send:   ['cancel', 'Отменить заявку',   'friend-action-btn friend-action-btn--muted'],
            cancel: ['send',   'Добавить в друзья', 'friend-action-btn'],
            accept: ['remove', 'Завершить дружбу',  'friend-action-btn friend-action-btn--remove'],
            remove: ['send',   'Добавить в друзья', 'friend-action-btn'],
        };
...
(function() {
    ...
    function update(idx) {
        ...
        items.forEach((item,i) => {
            ...
            if (off >  N/2) off -= N;
            if (off < -N/2) off += N;
            item.setAttribute('data-position', off);
        });
    }
    update(0);
    document.querySelector('.carousel-3d-btn.prev')?.addEventListener('click', () => update(cur-1));
    document.querySelector('.carousel-3d-btn.next')?.addEventListener('click', () => update(cur+1));
    const stage = document.getElementById('carouselStage');
    let wt = null;
    stage.addEventListener('wheel', e => { e.preventDefault(); clearTimeout(wt); wt = setTimeout(()=>update(cur+(e.deltaY>0||e.deltaX>0?1:-1)),30); }, {passive:false});
    let dx=null, moved=false;
    stage.style.cursor='grab';
    stage.addEventListener('mousedown', e => { dx=e.clientX; moved=false; document.body.style.userSelect='none'; });
    window.addEventListener('mousemove', e => { if(dx!==null&&Math.abs(e.clientX-dx)>5)moved=true; });
    window.addEventListener('mouseup',   e => { document.body.style.userSelect=''; if(dx!==null&&Math.abs(e.clientX-dx)>=50)update(cur+(e.clientX-dx<0?1:-1)); dx=null; stage.style.cursor='grab'; });
    stage.addEventListener('click', e => { if(moved)e.preventDefault(); }, true);
    stage.addEventListener('touchstart', e => { dx=e.touches[0].clientX; moved=false; }, {passive:true});
    stage.addEventListener('touchmove',  e => { if(dx!==null&&Math.abs(e.touches[0].clientX-dx)>5)moved=true; }, {passive:true});
    stage.addEventListener('touchend',   e => { if(dx!==null&&Math.abs(e.changedTouches[0].clientX-dx)>=50)update(cur+(e.changedTouches[0].clientX-dx<0?1:-1)); dx=null; });
...
// ── Переключение вид ─────────────────────────────────────────────────────
(function() {
    const KEY='games_view_mode';
    function applyView(v) {
        document.querySelectorAll('.games-view-icon-btn').forEach(b=>b.classList.toggle('active',b.dataset.view===v));
        document.getElementById('carouselView').style.display = v==='carousel'?'block':'none';
        document.getElementById('showcaseView').style.display = v==='showcase'?'block':'none';
    }
    applyView(localStorage.getItem(KEY)||'carousel');
    document.querySelectorAll('.games-view-icon-btn').forEach(btn=>btn.addEventListener('click',function(){localStorage.setItem(KEY,this.dataset.view);applyView(this.dataset.view);}));
})();
...
document.querySelectorAll('.showcase-tab').forEach(tab => {
    tab.addEventListener('click', function() {
        ...
        const t=this.dataset.showcase;
        const vi=document.querySelector('.games-view-icons');
        if(vi) vi.style.visibility=t==='achievements'?'hidden':'visible';
        document.getElementById('showcaseGamesBody').style.display=t==='games'?'flex':'none';
        document.getElementById('showcaseAchievementsBody').style.display=t==='achievements'?'block':'none';
    });
});
</script>
