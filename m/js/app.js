...
wish?.addEventListener('click', async () => {
  ...
  try {
    const r = await fetch('/m/api/wishlist.php', { method: 'POST', credentials: 'same-origin',
      body: new URLSearchParams({ game_id: wish.dataset.game, on: on ? 1 : 0 }) }).then(r => r.json());
    if (!r.ok) throw r;
...
if (sw) {
  ...
  sw.addEventListener('click', async () => {
    ...
    if (on) {
      const s = await current().catch(() => null);
      if (s) {
        await fetch('/chat/push_subscribe.php', { method: 'POST', credentials: 'same-origin', headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ endpoint: s.endpoint, unsubscribe: 1 }) }).catch(() => {});
