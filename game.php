...
$platformLabels = ['windows'=>'Windows','linux'=>'Linux','macos'=>'macOS',
                   'android'=>'Android','web'=>'Web'];
$platformStr = implode(', ', array_map(
    fn($p) => $platformLabels[$p] ?? ucfirst($p), $platforms
));
?>
<!DOCTYPE html>
<html lang="ru">
<head>
    ...
    *, *::before, *::after { box-sizing: border-box; }
    ...
    @media (max-width: 540px) {
        .gp-header { flex-direction: column; align-items: flex-start; padding-top: 16px; }
    ...
    #review-stars span:hover { color: #fbbf24; }
    ...
    .steam-btn:hover { filter: brightness(1.15); transform: translateY(-1px); }
    .steam-btn.icon-only {
        flex: 0 0 44px;
        padding: 0;
    }

    .steam-wrapper { position: relative; display: flex; }
    .steam-tooltip {
        position: absolute;
        bottom: calc(100% + 8px);
        left: 50%;
    ...
    .steam-wrapper:hover .steam-tooltip { opacity: 1; transform: translateX(-50%) translateY(-3px); }
    ...
    .gp-info-card h3 { font-size: .85rem; color: var(--muted); margin: 0 0 12px; text-transform: uppercase; letter-spacing: .06em; }
    .gp-info-row {
        display: flex;
        justify-content: space-between;
        gap: 12px;
...
(function() {
        ...
        .then(data => {
            ...
            if (myReview) {
                ...
                if (form && !document.getElementById('review-id')) {
                    ...
                    inp.type = 'hidden'; inp.id = 'review-id'; inp.value = myReview.id;
    ...
    document.addEventListener('click', e => {
        ...
        fetch(url, { method:'POST', headers:{'Content-Type':'application/x-www-form-urlencoded'}, body })
            .then(r => r.json())
            .then(d => { if (d.success) location.reload(); else alert('Ошибка: ' + (d.error || d.message)); })
            .catch(console.error);
    });
})();
