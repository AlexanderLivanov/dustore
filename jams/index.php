...
// Получаем username текущего пользователя для подстановки в псевдоним (приоритет: username -> telegram_username)
...
<script>
    ...
    // АВТО-ССЫЛКИ: преобразуем голые URL (http:// или https://) в кликабельные ссылки
    ...
    filtered.forEach(s => {
        const phase = s.phase || 'finished';
        const phaseText = phaseMap[phase] || phase; // русское название
    ...
    function openView(id) {
        ...
        viewModal.innerHTML = `
            ...
            <div class="modal-actions">
                ${actionButton}
                ${rateButton}
                <button class="btn-team" onclick="event.stopPropagation(); window.location.href='/l4t';">Команда</button>
...
<button class="btn-share" onclick="event.stopPropagation(); shareSprint(${sprint.id})">Поделиться</button>
    ...
    // ===== ИЗМЕНЁННАЯ ФУНКЦИЯ startJoin =====
    function startJoin(sprintId) {
        // Проверка авторизации
        if (!isLoggedIn) {
            if (confirm('Для участия в спринте необходимо авторизоваться. Перейти на страницу входа?')) {
    ...
    // ===== ИЗМЕНЁННАЯ ФУНКЦИЯ openRegister (дополнительная защита) =====
