...
// Получаем studio_id текущего пользователя (предполагается, что в сессии или глобально)
// Если $studio_id не определён в header, возьмём из сессии
if (!isset($studio_id) && isset($_SESSION['USERDATA']['studio_id'])) {
    $studio_id = (int)$_SESSION['USERDATA']['studio_id'];
}
if (empty($studio_id)) {
    echo '<div class="alert alert-danger">Студия не найдена</div>';
...
if ($selected_id) {
    ...
    // Агрегируем детальные оценки (исправленный запрос)
...
<div style="display:grid;grid-template-columns:260px 1fr;gap:16px;align-items:start;">
    ...
    <div>
        ...
        ?>
            ...
            <!-- Детальные оценки (GQI и критерии) -->
